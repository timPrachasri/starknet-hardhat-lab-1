...
import type { KeyPair, Signature, Contract } from "starknet";
...
export class Signer {
  ...
  async sendTransaction(
    ...
    if (nonce === 0) {
      const { nonce: _nonce } = await caller.call("get_nonce");
      nonce = _nonce as unknown as number;
    }
